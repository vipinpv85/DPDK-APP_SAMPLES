...
extern struct element e_ptr[16] __rte_cache_aligned;
...
worker_generic(void *arg)
{
	...
	setTraceInfo(lcore_id, __func__, NULL, __LINE__);
...
worker_generic_burst(void *arg)
{
	...
	setTraceInfo(lcore_id, __func__, __FILE__, __LINE__);
	...
	while (!fdata->done) {
		...
		executeTrace(lcore_id, RTE_DIM(events), (void *) &events, nb_rx);
