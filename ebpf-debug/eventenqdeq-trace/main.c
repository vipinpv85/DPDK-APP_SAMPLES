...
extern uint16_t app_argument_count;
extern char app_argument[DPDK_MAXARGS][32];
extern uint16_t argument_count;
extern char argument[DPDK_MAXARGS][64];
...
static int
myDisplay (__attribute__((unused)) void *arg)
{
	display();
	return 0;
}
...
main(int argc, char **argv)
{
	...
	err = rte_eal_init(argument_count, args);
	if (err < 0)
	...
	for (int j = 0; j < app_argument_count; j++)
	{
		args[j] = app_argument[j];
	}
	...
	/* Parse cli options*/
	parse_app_args(app_argument_count, args);
	...
	RTE_LCORE_FOREACH_SLAVE(lcore_id) {
		...
		if (!fdata->rx_core[lcore_id] &&
			...
			if (displayLaunch == 0) {
				err = rte_eal_remote_launch(myDisplay, NULL, lcore_id);
				if (err)
					rte_panic("Failed to launch display on core %d\n", lcore_id);
				displayLaunch = 1;
				continue;
			} else
				continue;
